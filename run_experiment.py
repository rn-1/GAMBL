...
def make_exp_name(params: dict) -> str:
    ...
    if dataset == 'csv':
        ...
        return f"{model}_csv_{csv_stem}_wd{wd}_frac{frac}_seed{seed}{size_sfx}{drop_sfx}"
    ...
    if dataset == 'subsequence':
        ...
        return (
            ...
            f"_k{subseq_len}"
            f"_n{subseq_samples}"
            f"_wd{wd}_frac{frac}_seed{seed}{size_sfx}{drop_sfx}"
        )
    ...
    if dataset not in ('modular_arithmetic',):
        return f"{model}_{dataset}_wd{wd}_frac{frac}_seed{seed}{size_sfx}{drop_sfx}"
